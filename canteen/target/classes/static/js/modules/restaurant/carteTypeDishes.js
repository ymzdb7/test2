...
$(document).ready(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'restaurant/carteTypeDishes/list/'+id,
        datatype: "json",
   
        colModel: [			
			{ label: 'id', name: 'id', width: 30, key: true},
			{ label: '菜品id', name: 'dishesId', width: 70 }, 	
			{ label: '菜品种类', name: 'typeName', width: 70 }, 
    ...
    $('#search_panel').change(function (){
    	...
    	if(val == ''){
    		...
    		lastVal = val;
        	for(var i=0;i<dishesArr.length;i++){
        		var dishName = $(dishesArr[i]).html();
        		if(dishName.indexOf(val) != -1){
        			$(dishesArr[i]).css('color','red');
        		}
        		else{
        			$(dishesArr[i]).css('color','');
        		}
	...
	methods: {
		...
		del: function () {
			...
			/*confirm('确定要删除选中的记录？', function(){*/
				$.ajax({
					type: "POST",
				    url: baseURL + "restaurant/carteTypeDishes/delete",
		...
		go: function(){
			location.href="/modules/restaurant/carte.html?token="+localStorage.getItem("token");
			},
		 //跳转到evaluate.html
     /*   evaluate:function(){
		...
		saveOrUpdate: function () {
			...
			var url = id != null ? "restaurant/carteTypeDishes/save/"+id: "restaurant/dishes/update";
			var ides=[];
...
function getDishesById(){
    ...
    $.ajax({
		url:"/restaurant/carteTypeDishes/getDishesByIdinfo/"+id,
		type:"post",
		success:function(data){
				$('input:checkbox[name=idList]').each(function() {
						 for (var i = 0; i < data.carteTypeDishes.length; i++) {
						  if($(this).val()==data.carteTypeDishes[i].dishesId) {
...
function addTiao(){
	...
	location.href="/modules/restaurant/dishes.html?token="+localStorage.getItem("token"); //跳转到菜品页面
